...
c reweight factors when Ws decay, that include branching ratios
...
c
      ...
      elseif(prdct.eq.'ww')then
        ...
        write(*,*) 'tot5a: non implemented final state', prdct
        stop
      endif
...
c
      ...
      implicit none
      real * 8 tot5b,s,xx,xy,xcth1,xth2,xjac
      real * 8 pi,pi2,zero,hc2
      parameter (pi=3.14159265358979312D0)
      parameter (pi2=pi*pi)
      parameter (zero=0.d0)
...
c
      ...
      elseif(prdct.eq.'ww')then
        ...
        write(*,*) 'tot5b: non implemented final state', prdct
        stop
      endif
      xnormsv = ze2**2 * xjac/(16*pi2)/(16*pi)
      xnormborn = ze2**2 * xjac/(16*pi)
...
c Insert here W and Z branching ratios; that for Z is meant up to 
c coupling factors (see the definition of dec2cf)
          brrv1msb=0.d0
          brrv2msb=0.d0
...
c coupling factors (see the definition of dec1cf and dec2cf)
          ...
          brrv2msb=dec2cf*brrv2msb
        elseif(prdct.eq.'ww')then
c Move this condition outside the if statement when decays in WZ and ZZ
c will be implemented
